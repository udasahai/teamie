...
const timeout = config.timeout //time out in seconds 
...
const fillUserGroupEntityTotal = async(props) => {
    ...
    //See if we can fill table 2 by comparing the site_id, user_id and tracking_type fields
    // if (req.body.user_id == previousPing.user_id && req.body.site_id == previousPing.site_id && req.body.entity_type == previousPing.entity_type &&
    //     req.body.entity_id == previousPing.entity_id && req.body.group_id == previousPing.group_id) {
    ...
    let args = [previousPing.site_id, previousPing.user_id, previousPing.group_id, previousPing.entity_type, previousPing.entity_id]
    ////console.log(mysql.format(query_string, args))
    let res = await db.query(query_string, args);
    let prev_time = 0;
    ...
    args = [previousPing.site_id, previousPing.user_id, previousPing.group_id, previousPing.entity_type, previousPing.entity_id,
...
const fillUserGroupTypeTotal = async(props) => {
    ...
    //See if we can fill table 2 by comparing the site_id, user_id and tracking_type fields
    // if (req.body.user_id == previousPing.user_id && req.body.site_id == previousPing.site_id && req.body.tracking_type == previousPing.tracking_type) {

    ////console.log("same tracking type")
    let query_string = `SELECT total_time FROM user_group_type_total
                                    WHERE site_id = ? and user_id = ? and group_id = ? and tracking_type = ?`
    let args = [previousPing.site_id, previousPing.user_id, previousPing.group_id, previousPing.tracking_type]
    ////console.log(mysql.format(query_string, args))
    let res = await db.query(query_string, args);
    let prev_time = 0;
    ...
    args = [previousPing.site_id, previousPing.user_id, previousPing.group_id, previousPing.tracking_type, total_time, insertId, { last_track_id: insertId, total_time: total_time }]
    return await db.query(query_string, args);
    // }
